...
| `v5`    | `iou_mt`         | N threads, each owns an `io_uring` ring with multishot `recvmsg` + provided buffer ring. One `io_uring_enter` per drain cycle (amortized across many CQEs). See `v5_iou_mt/RESULT.md`                     | Kernel stack still traversed; `sk_buff` still allocated per packet                         |
